...
double calculator(string expression){
    ...
    for (int i = 0; i<=expression.length()-1; ++i){
        ...
        else if (expression[expression.length()-1]== 't') {
            test = true;
        }
    }
    if (!numbers_sorted.empty() && ops_sorted.empty())
    {
        onlynum = true; 
        cout<<numbers_sorted[0]<<endl;
        return numbers_sorted[0];
    }
    vector<bool> used(numbers_sorted.size() + 1, false); //honestly i dont know how it works but it works
    ...
    if ((final_numbers.size() != final_ops.size() + 1) && !onlynum) {
        cout << "Jumlah operator tidak valid. Contoh: '2+-2' dapat ditulis sebagai '2-2'." << endl;
        return -1; 
    }
   
   if (expression[expression.length()-1]== 't') {
       test = true;
   }
   if (test) {for (int t = 0; t < final_numbers.size(); t++) {
        cout << "hasil postfix: "<<final_numbers[t] << " ";
    }
...
// commented code below is currently not working
/*
for (int iter = 0; iter<=strresult.length()-1; ++iter){
    if (strresult[iter]=='.') behindC = true; 
    if (behindC && DAC<8) { 
        DAC++;
        if (strresult[iter] == '0' && iter<strresult.length()) {
            for (int trail = 0; trail<=6-iter; ++trail){
                if (strresult[iter+trail]=='0')
                    {
                        break;
                    }
    ...
    cout << fixed << setprecision(DAC) << result;
*/
